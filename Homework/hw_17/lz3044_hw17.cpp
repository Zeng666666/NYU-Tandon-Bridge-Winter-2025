            ...
            if (parent == grandparent->left) {
                ...
                if (getColor(uncle) == RED) {
                    ...
                    if (curr_node == parent->right) {
                        // Case 2: left-right 
                        curr_node = parent;
                        // Find reference to parent in grandparent
                        RBTNode<T> **parent_ref = &(grandparent->left);
                        singleCCR(*parent_ref);
                        parent = curr_node->parent;
                    }
                    // Case 3: left-left
                    // Find reference to grandparent
                    RBTNode<T> **grandparent_ref;
                    if (grandparent->parent == nullptr) {
                        grandparent_ref = &root;
                    }
                    else if (grandparent->parent->left == grandparent) {
                        grandparent_ref = &(grandparent->parent->left);
                    }
                    else {
                        grandparent_ref = &(grandparent->parent->right);
                    }
                    singleCR(*grandparent_ref);
                    break;
                }
            }
            else {
                ...
                if (getColor(uncle) == RED) {
                    ...
                    if (curr_node == parent->left) {
                        ...
                        curr_node = parent;
                        // Find reference to parent in grandparent
                        RBTNode<T> **parent_ref = &(grandparent->right);
                        singleCR(*parent_ref);
                        parent = curr_node->parent;
                    }
                    // Case 3: right-right
